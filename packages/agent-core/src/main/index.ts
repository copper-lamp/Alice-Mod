 ...
 * 兼容两种来源格式：
 * <ul>
 *   <li>JE 端（SchemaGenerator）：{name, description, category, input_schema, output_schema, execution}</li>
 *   <li>BE 端（ToolRegistry）：{name, description, category, parameters, enabled}</li>
 * </ul>
 * 输出统一为 ToolSchema：{name, description, parameters, category, enabled}。
 */
function normalizeToolSchema(raw: Record<string, unknown>): ToolSchema {
  // 1. name
 ...
 * 支持嵌套 object / array。
 */
function jsonSchemaToParams(schema: Record<string, unknown>): ToolSchema['parameters'] {
  ...
  for (const [key, def] of Object.entries(properties)) {
    ...
    result[key] = {
      ...
      ...(def.properties ? { properties: jsonSchemaToParams(def.properties) } : {}),
      ...(def.items ? { items: convertSingleParam(def.items as Record<string, unknown>) } : {}),
    }
  }
  return result
}
...
function convertSingleParam(def: Record<string, unknown>): ToolSchema['parameters'][string] {
  return {
...
// 导出归一化函数供回归测试使用（IT-V2 B4）
export { normalizeToolSchema, jsonSchemaToParams, convertSingleParam }
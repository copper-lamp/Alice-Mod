...
2. 确认最大图尺寸可作为统一源图
3. 生成标准化 `build/icon.png`
4. 生成多尺寸 `build/icon.ico`
5. 将 `package.json` 中的 `build.win.icon` 从 `build/icon.png` 改为 `build/icon.ico`
...
2. 检查 `package.json` 是否引用 `build/icon.ico`
3. 重新执行 Windows 打包

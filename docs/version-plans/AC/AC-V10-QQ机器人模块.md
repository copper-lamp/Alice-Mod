...
#### 2.2.4 NapCatManager — NapCat 托管管理器 (Phase 2)
...
interface NapCatManager {
  ...
  // 配置
  updateConfig(config: NapCatManagedConfig): Promise<void>;
...
| 阶段 | 版本 | 功能 | 状态 |
|------|:----:|------|:----:|
| Phase 1 | V10 | 外部模式 + OneBot 客户端 + QQ 工具 + 桥接 + 权限 | **本次实现** |
| Phase 2 | V10.1 | NapCat 托管管理器（自动下载/子进程管理/配置注入） | 规划中 |
| Phase 3 | V10.2 | 进程监控 + 自动恢复 + 崩溃告警 | 规划中 |
| Phase 4 | V10.3 | 二维码内置显示 + 一键登录交互 | 规划中 |
| Phase 5 | V10.4 | 自动更新 + 多实例支持 | 规划中 |
